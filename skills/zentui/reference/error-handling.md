...
| 3 | `ExitAuthError` | Authentication failure | Set env vars or run `zentui auth login` |
...
2. Check stored credentials: `zentui auth status -o json --profile PROFILE`
3. Re-authenticate: `zentui auth login --method token --subdomain SUB --email EMAIL --api-token TOKEN`

## Common error scenarios

| Scenario | Exit code | Resolution |
|----------|-----------|------------|
| No credentials configured | 3 | Run `zentui auth login` or set env vars |
| Expired OAuth token | 3 | Re-run `zentui auth login` to refresh |
| Ticket ID doesn't exist | 5 | Verify ID; ticket may have been deleted |
| Invalid flag value | 2 | Check `zentui schema --command "..."` for valid values |
